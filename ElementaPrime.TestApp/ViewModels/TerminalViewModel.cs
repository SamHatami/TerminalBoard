...
public class TerminalViewModel : PropertyChangedBase, ITerminalViewModel, IHandle<CanvasZoomPanEvent>, IHandle<OutputUpdateEvent>
{
    ...
    public TerminalViewModel(IEventAggregator events, IOperator @operator) //TODO: Handle all communication with the terminal through a TerminalService?
    {
        _events = events;
        Operator = @operator;
    ...
    private void Initialize()
    {
        ...
        if(Operator.InputSockets != null)
        {
            foreach (var inputSocket in Operator.InputSockets)
            {
                var socket = new SocketViewModel(this, _events, DataPortDirection.Input, inputSocket)
                {
        ...
        if (Operator is ValueOperator<float> floatValueTerminal)
        {
            GetInputValue = true;
        ...
        if (Operator is ValueOperator<int> intValueTerminal)
        {
            GetInputValue = true;
        ...
        if (Operator is IOutputOperator outputTerminal)
        {
    ...
    {
        if (Operator is ValueOperator<float> floatValueTerminal && float.TryParse(value, out float floatValue))  //TODO : detta fungerar inte generellt
        {
            foreach (var wire in Operator.Connections)
            {
